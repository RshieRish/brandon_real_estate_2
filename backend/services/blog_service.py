...
def _upload_image(image_bytes: bytes, mime: str = "image/jpeg") -> Optional[str]:
    """Upload generated bytes to R2; return the public URL, or None on failure.

    Returns None (not a placeholder) on failure so the frontend renders its
    on-brand empty state instead of a generic placeholder image.
    """
    ...
    print("[Blog] All image storage methods failed — returning None; frontend will render empty state.")
    return None
...
- Use H1 (#) anywhere except the title"""
    ...
    def generate_blog_image(title: str, max_attempts: int = 2) -> Optional[str]:
        ...
        Returns the R2 URL on success, or None on any failure. The caller writes
        None into the DB column and the frontend renders the on-brand empty
        state (gradient + book icon) — no third-party placeholder.
