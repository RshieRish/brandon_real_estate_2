...
### 2026-08-27 - Sydney Command Recovery Rollout Through Durable Drain Hotfix
...
- The guarded production selector passed a content-free dry-run and enqueued exactly once. Run `9e2f123a-23db-4c48-9a87-b757e11c5e4a` invoked only `command_contact_audience_preview`, whose backend ledger reached `succeeded`; no Gmail, Workspace, calendar, CRM, Command mutation, or message send occurred. Its local `tool_after_bundle` remained intact and never attempted, exposing that one live-tail SQLite error could escape the synchronous drain and permanently terminate the background drain worker.
- The production defect is now fixed test-first on `codex/sydney-spool-drain-recovery`: live-tail SQLite errors are isolated from durable delivery and the worker retries after any failed iteration instead of dying. The exact changed-area matrix passes `250` with `5` environment skips and `3` subtests; focused Ruff, compileall, diff checks, and independent review are clean. Final recovery remains incomplete until this hotfix is merged/deployed and the same saved spool record reaches one terminal response without a new enqueue.
