...
- Test-first source/internal regressions and the broader local Command/Agent Control gate pass `496` tests with `17` existing deprecation warnings. The complete exact Hermes/MCP/spool/retry gate passes `305` tests plus `3` subtests; focused contact UI/decoder tests pass `128`. The CI inventory now includes note/section/service/router regressions; its guard passes `7` tests. Candidate projection is not yet a deployed browser result; PR, CI, and production evidence are recorded when confirmed.
