...
- Test-first evidence: 22 focused failures plus two exact sequential/concurrent Hermes seam failures before implementation. The final exact Hermes/template gate passes 390 tests plus three subtests; the unchanged 61-file PostgreSQL16/TLS matrix passes 2,231 tests with 41 existing warnings in 184.95s. The new preflight suite is an additive required CI test with RED/GREEN inventory coverage. Runtime and preflight/provider test lint pass; eight unchanged deployment-test lint diagnostics are independently reproduced on origin/main and were not expanded into unrelated edits.
- Isolated verification now calls the real installed preflight and successful-completion helpers while detaching only ledger/delivery hooks. Its nine offline tests plus nine subtests prove blocked previews cannot execute and missing/truncated/wrong-ID/historical skill evidence cannot pass as current; actual model-visible call/result/completion IDs and exact skill bytes must match. This is prepared verification, not a deployed result. PR/main CI, installed release checks and the repeated read-only model probes remain required.

### 2026-09-05 - Command Data Repair Verified in Production; Narrow Sydney Follow-up
- PR #39 merged at `032169157992f8092e0f4015f21f3e071f88e7a7`. Complete PR run `33990304001` attempt 2, main run `33992595123`, and migration-chain runs are green. The final PR PostgreSQL matrix passed 2,231 tests; main passed all three required jobs. The one exact-SHA rerun followed an independently reproduced pre-existing 250ms database/TLS deadline-fixture race; no test, production timeout, or guard was relaxed.
- Backend deployment `4f6faf11-0f13-4cb0-86c1-8c2d4fc4c9a8`, worker `d42fa2df-6a3c-4ea9-b9ce-ae54683b7363`, and Vercel production `6285916724` are successful on the exact merge. Atlas generated deployment `78b72a81-c419-4614-84d9-f09c7115d161` is successful with all four installed asset hashes matching reviewed source. All three live health/readiness checks pass; live JSON-RPC preserves exactly 27 ordered tools and existing request UUID requirements.
- After verified protected full-database and repair-specific backups, the reviewed additive fingerprint `ee294f143176725790504984fb4325144a942c2cb6e3019c76973197197d58eb` committed 207 recovered addresses: 205 structured and two explicitly needing review. Installed verification checks every field, source coordinate, and all 207 audit records; a repeat dry run proposes zero additions. No manual address was overwritten. The authenticated existing September draft was explicitly refreshed once to version 2: 13 ready addresses, one genuinely missing, 14 original recipients, zero exclusions. Recipient/name/message/design content hash remains `3ae7c2779cd8277fdd5d197f0e7bc5f1f641e514798c316347bb4a9a05927f11`; no approval, delivery attempt or provider receipt exists.
- Installed all-317-contact timeline comparison passes: 5,963 recovered display rows, 594 proven controls filtered, 5,952 source-dated rows, 5,922 source times and 27 activity groups; zero footer/profile residue or review-needed rows. All 2,653 non-timeline occurrences read correctly, including 178 notes (168 bodies, 10 title-only), 2,052 captured task dates and 79 opportunity budgets. The genuine partial SmartPlans capture remains explicitly partial. All 12,580 archive files match recorded size/hash, including 169 DocuSign artifacts in 11 folders, 149 ZIP bundles and 512 PDF members; three actual private downloads match original member bytes.
- Authenticated production browser acceptance covers the recovered note and captured date on contact 149, task dates/count separation/SmartPlan names on contact 52, opportunity budget versus uncaptured value on contact 58, and readable plus expandable saved-search criteria on contact 75. The archive filter, nested original folders, ZIP PDF listing and the refreshed existing campaign are verified in the live UI. Original source, timeline and note table hashes are unchanged; all three unauthenticated private-content probes return 401.
- Installed Sydney birthday-only and combined model probes pass in 12.51s and 18.04s, preserving full sample names, current counts and dates without technical audience identifiers in the returned or persisted assistant reply. A new anniversary-only probe returns correct fresh data but twice skips the required current skill read. A narrow test-first nonterminal current-skill preflight is in progress on `codex/sydney-current-skill-preflight`; do not claim every prompt passes yet. The six installed active/history/test-opt-in reads pass: default results exclude the observed completed and controlled-test tasks while explicit history/test reads retain them.
- Visible Telegram history remains 54 messages through 1367 with SHA-256 `ee4fe291e0f93d7bb62650f1b37c95b1ca8813c1edcf65ed6373545522c26f9e`; no active Telegram run is observed. Isolated model probes write no live chat/ledger and are not Telegram delivery receipts. No reset, migration, archive replay, outreach, card order, document signing or provider reconnection occurred. Send Out Cards remains disconnected/contract_required; historical addresses are not a guarantee of current deliverability.

### 2026-09-05 - Command Data Presentation and Sydney Repair Release Gate (pre-release record)
- The five Linux reference images are now installed byte-for-byte from reviewed run `33989816348` / artifact `9976331638`; all original/retry1/retry2 images were byte-identical, and independent native-size review confirms intended-only changes. No comparison threshold, fixture mask, or application logic changed. The exact local PostgreSQL matrix now passes 2,231 tests (41 warnings, 185.90s), the deployment contracts pass 9, and DocuSign desktop/mobile browser cases pass 2/2 with fictional filenames. Those archive cases are now additive CI gates. Complete green CI on the final image commit and installed production checks remain required.
- PR #39 source `f7603d1` passed the remote PostgreSQL matrix (2,230 tests, 43 warnings, 613.03s), exact Hermes gate, migration-chain gate and Vercel preview. Its browser gate caught five stale contact-detail image baselines reflecting the intended summary-strip label/height and Bookings count. All five changes were visually reviewed; only the five Darwin baselines were regenerated, and the complete Darwin visual gate passes 14/14 without changed thresholds or fixtures. Linux baselines still require runner-produced image review. A test-first CI follow-up preserves only failed synthetic contact actual/diff PNGs for three days, excluding traces and authentication state. Production remains unchanged until complete green final CI.
- The user authorized repairing the five audited domains. This continues completed Task 8/9 and PR #38; it does not repeat the original archive import. Development is isolated in the existing repair worktree and preserves the unrelated root checkout changes.
- Address recovery is additive, source-coordinate checked, fingerprinted, and protected against concurrent/manual CRM edits. The reviewed dry run proposes 207 addresses (205 structured, two formatted-only requiring review), including 13/14 September recipients; one recipient has no archived mailing address. A complete private custom-format production backup is verified outside the repository before any apply. No address has been applied yet.
- Timeline presentation reads owned, size/hash-verified canonical HTML or accessibility snapshots. Real date headings retain date-only/wall-clock precision; original timestamps and source records stay unchanged. It recovers activities embedded in navigation captures, handles the old quoted-generic omission through exact legacy projections, protects literal date/footer/Reply text, and exposes unsupported activity as review-needed instead of false empty. The frozen candidate passed the read-only all-317-contact comparison: 5,963 recovered display rows, 594 proven control-only captures filtered, 5,952 source-dated rows, 5,922 source times and 27 activity groups, with zero footer/profile residue, review-needed rows, duplicate page keys or ORM writes. This is not yet installed production evidence.
- Contact sections now expose captured task dates, separate budget from transaction value, show actual SmartPlan names and readable saved-search criteria with complete expandable evidence, and split internal/recovered counts without unsafe same-title deduplication. The campaign's explicit Check updated addresses action fills only missing frozen address snapshots and invalidates approval without sending.
- The authenticated archive browser exposes original folders, nested bundles and exact-index downloads. All 169 imported DocuSign artifacts, 11 source folders, 149 ZIP bundles and 512 contained PDFs have verified bytes; the global 12,580-file catalog remains available. This is archive visibility, not a reconnected DocuSign account or signing action.
- Sydney's ordinary celebration replies omit technical audience metadata but honor genuine explicit requests. Actual current-request pinning survives Hermes compression and synthetic continuations, and already-sanitized stored replies are cleaned without restoring secrets. Active task reads filter completed/archived/explicit controlled-test records before limits; history remains an explicit option. Independent Sydney review approved; the exact runtime/CRM/MCP gate passed 368 tests plus three subtests, preserving the ordered 27-tool surface and managed skill 2.2.3.
- Final development verification: complete frontend 721 tests, TypeScript and the 37-route production build pass; full built-browser inventory passes 55 with eight project-inapplicable skips and no failures. This includes repaired 44px mobile task-selection targets. The frozen 61-file PostgreSQL16/TLS release matrix passes 2,230 tests with 41 existing warnings in 192.55s. The exact pinned Hermes/template gate passes 358 tests plus three subtests; final independent address/timeline review passes 276 tests and reports no blockers. All domain reviews approve. Critical whole-tree Ruff, backend-configured new-module Ruff, and diff checks pass. A wider diagnostic suite retains separately documented baseline/environment failures and is not called universally green.
- Release is not yet deployed. PR/main CI, installed-service verification, additive address apply, authenticated browser checks and isolated deployed-model checks are still required. No migration, archive replay, reset, Telegram test message, outreach, card order or document signing has occurred in this repair.

### 2026-09-05 - Read-only Command and Sydney Post-release Audit
- Result: **not ready for a blanket all-data/all-prompts sign-off**. No application or production changes were made. Preserved the existing root checkout and release-evidence edits in this worktree.
- Full archive check: 12,580/12,580 files retain bytes and match recorded size/SHA-256; 317 recovered profiles/positions, 2,536 sections and 9,210 occurrences have no row-count or cross-contact ownership mismatch. Rechecked all 178 repaired notes (168 bodies, 10 title-only, 19 corrected titles) with the unchanged source/note hash and zero ORM mutations. All 2,653 non-timeline source adapters return; six contacts pass all seven section reads and workspace-summary calls. One SmartPlans capture, contact 292, remains explicitly partial.
- Remaining data defects: 207 contacts retain non-placeholder mailing-address text in captured profile/timeline blocks, including 13/14 September campaign recipients, but the structured address table has zero rows. All 2,052 captured task rows have due_date while the display reads due_at; 79/127 opportunities retain budget while the display only reads value_cents. These require source-backed interpretation, not blind conversion or invented data.
- Timeline remains semantically incorrect: strict navigation/profile/footer patterns flag an overlap-aware 840/6,557 events across all 317 recovered contacts; all events lack structured timestamps even where raw date text exists. The extractor uses bare event-kind page labels and can include unrelated trailing content. Original archive/source rows were not changed.
- Browser checked live Notes, Timeline, Tasks, SmartPlans, Opportunities, Saved Searches, Source Evidence and empty internal Bookings on representative contacts. Header clearance and bounded cards are improved, and the inspected desktop page has no horizontal overflow. Remaining UX issues include same-title captured/internal task counts (contact 52: 20 vs 10 matching rows per side), plan-number labels, raw saved-search JSON and misleading not-captured field labels. Candidate same-title/unlinked internal task matches total 453 over 59 contacts and require provenance review before any deduplication. Browser extension error is separate from application health; no phone-size certification was performed.
- Deployed backend, Atlas gateway and worker health pass; live tools/list has exactly 27 expected tools with no forbidden tools. Five read-only real-model cases with six copied prior messages completed: combined September 8.53s, birthday-only 6.66s, anniversaries-only 9.50s, Command/audience/tasks 14.67s, and Workspace/Gmail/Drive/Calendar 17.15s. All called their expected tools; current names were preserved. Birthday-only still exposed technical audience metadata; task results included completed and rollout-test records. Preserve these failures instead of calling every prompt green.
- No real Telegram jobs exist after the final release as of 16:34 UTC; no queued/running/retry job was observed. Existing visible chat is unchanged at 54 messages/hash `ee4fe291e0f93d7bb62650f1b37c95b1ca8813c1edcf65ed6373545522c26f9e`; durable storage retains 1,380 Telegram events and 73 checkpoints. Isolated read timings exclude delivery/queue latency, and no reset, Telegram message, draft, outreach or card order was performed. Send Out Cards still reports disconnected/contract_required; the existing campaign has 14 recipients, zero ready addresses and no delivery receipts.
- Evidence: `docs/audits/2026-09-05-command-sydney/` contains canonical artifact, standalone report, reviewed prompt outcomes, read-only SQL and an inspectable notebook. The packaged report passes canonical validation, desktop/narrow reader checks (1440/390), source-dialog check and exact artifact-payload verification. Notebook cells preserve the executed production queries but are explicitly not presented as locally executed cells. Further fixes and a controlled real Telegram delivery check need user direction.

### 2026-09-05 - Recovered Notes and Fresh Sydney Names Verified Live
- PR #38 merged at `cd77abbb1395e5090ae129462d84ea5e4835b9e6` after both independent reviews and complete green CI: PostgreSQL 16/TLS `1,884 passed` with `38` existing warnings in `579.07s`, exact Hermes/MCP/spool/retry `313 + 3` subtests, and the complete frontend/browser gates. The runner-only follow-up retains every test and safeguard and gives the database job a bounded 30-minute allowance after two earlier runner timeouts.
- Production backend `02dbaa6a-661f-4a91-9851-4f9e8334715e` and worker `a1df595a-8de9-41ec-839f-1e6a89db8aff` reached `SUCCESS` at the exact merged SHA. Atlas generated release `1fa04f84-b2d5-4e6c-9017-321696c63d0b` reached `SUCCESS`; installed provider, managed skill `2.2.2`, and MCP hashes match the reviewed source. All three health/readiness probes pass. Live JSON-RPC proves exactly `27` ordered unique tools, original `22` unchanged, request UUID requirements intact, and no forbidden tool.
- The final live Gemini probe used the deployed provider and skill without candidate overrides, six copied existing-chat messages, and a disposable session/spool. It refreshed the stored instruction cache, called only the skill viewer and authoritative celebration preview, and returned all five actual sample names and the current `1` birthday / `13` home anniversaries / `14` unique contacts / `0` complete mailing addresses in `8.79s`. Reply assertions confirm current-turn evidence, preserved names, and omitted technical audience identifiers. This isolated read-only probe is not a Telegram message or fabricated delivery receipt.
- Visible history before and after deployment/probes remains `54` messages through ID `1367`, SHA-256 `ee4fe291e0f93d7bb62650f1b37c95b1ca8813c1edcf65ed6373545522c26f9e`. No reset, new session, live conversation edit, card order, outreach, archive reapply, migration, or CRM mutation was performed.
- Fresh deployed-code verification still reads all `178` recovered notes across `154` contacts: `168` bodies, `10` title-only notes, `19` corrected Updated titles, and `178` correctly presented internal notes. The original source/note SHA-256 remains `443f3adb79607a1c9f2db409f4f9f1ba39fdc386798b251cbae0a0399dd8cf21`, with zero ORM writes. The authenticated production browser was refreshed after final deployment and again shows note #107's actual text in both recovered and internal Notes; the prior Updated/long-body and title-only browser checks also remain recorded below. This repair is complete.

### 2026-09-05 - Sydney Existing-Chat Celebration Freshness Follow-up
- PR #38's database gate encountered runner time limits twice, not failed assertions: attempt 1 reached `65%`, and attempt 2 reached `99%` before the unchanged `15`-minute job deadline. The same database code and full test inventory passed in `551.49s` for PR #37. A test-first deployment contract now gives only this PostgreSQL matrix a bounded `30`-minute allowance; no test, TLS guard, cleanup, migration check, or application timeout is removed or relaxed. Release still requires the complete green rerun and fresh deployed verification.
- PR #37's backend and note repair is live, but the copied-existing-history model check exposed a separate stale-answer path: Sydney repeated old masked preview text with no current tool call, even when the real durable-context prompt block was preserved. No live chat or ledger was written by these isolated checks.
- Added a concise current-turn authoritative celebration lookup requirement to the identity-scoped always-loaded provider instructions. Historical-only explanations remain historical; current names must come from the authorized preview, never inferred masks. The managed skill `2.2.2` mirrors this distinction without changing tool permissions, mutation gates, or session/history storage.
- Test-first provider contracts fail before the rule and pass afterward (`136` provider tests); the actual pinned `AIAgent`/`SessionDB` regression now wraps the real prompt builder and proves the provider instructions replace the obsolete cached prompt without starting a new session or changing history. The full exact Hermes/MCP/spool/retry gate passes `313` tests plus `3` subtests.
- The identical read-only live-Gemini request, using six copied existing-history messages and the candidate provider instructions, calls only skill view and authoritative celebration preview and returns all five actual sample names in `9.03s`. This is candidate evidence, not a Telegram delivery or the final deployed follow-up. Original visible history remains `54` messages through ID `1367`, SHA-256 `ee4fe291e0f93d7bb62650f1b37c95b1ca8813c1edcf65ed6373545522c26f9e`.

### 2026-09-05 - Recovered Note Content and Private Celebration Names Repaired
- Traced the live contact #149 / note #107 failure through source, stored note, API, and browser. All `178` recovered notes stored intact historical `values.raw_lines`, while the section read path expected `values.body`; `19` updated notes also inherited the metadata word Updated as their title. The linked internal notes displayed the capture's author/control text verbatim.
- Added a strict read-only historical-note adapter with structured-field precedence, validated metadata/control boundaries, full text bounds, and explicit title-only handling. Internal presentation uses one batched ownership/provenance query, cleans only an exact unchanged imported body, and preserves later edits, conflicting/unproven evidence, ORM state, archive bytes, source identities, and timestamps. No data migration or archive reapply is needed.
- Read-only candidate verification against all live recovered notes returns `168` bodies, `10` title-only notes, and `19` corrected titles with zero errors. It cleans `178` internal responses across `154` contacts without dirty ORM state. The original `178` source/note baseline hash is `443f3adb79607a1c9f2db409f4f9f1ba39fdc386798b251cbae0a0399dd8cf21`.
- The protected celebration preview now returns actual contact names within the existing five-example bound. General-audience masking, counts, checksum semantics, authentication, and content-free audit remain unchanged. The managed skill shows names/dates and a complete review link, keeps technical audience metadata out of ordinary replies, and cannot represent an internal draft as connected physical fulfillment.
- Test-first source/internal regressions and the broader local Command/Agent Control gate pass `503` tests with `17` existing deprecation warnings after review hardened malformed raw-capture preservation and maximum-length newline bodies with optional suffixes. The complete exact Hermes/MCP/spool/retry gate passes `305` tests plus `3` subtests; focused contact UI/decoder tests pass `128`. The CI inventory now includes note/section/service/router regressions and a non-secret, no-provider-call Gemini initialization fixture required by the full application import. Candidate projection is not yet a deployed browser result; PR #37, CI, and production evidence are recorded when confirmed.
- PR #37 merged at `d15e40b8760570112f2890f57ed7ee6e1e5004ef` after the final PostgreSQL 16/TLS matrix passed `1,883` tests, exact Hermes `305 + 3` subtests, complete frontend `687`, and production-build browser `45`. A remaining historical masked-name assertion was corrected; all `6` transactional tests also pass independently on isolated PostgreSQL 16.13/TLS, with the test-only delta independently reviewed.
- Production backend `5ff43ce6-e4ff-4adb-86cf-29aee520b18f` and worker `45d7fce2-0d60-4db9-ba7e-a71b21953e54` are `SUCCESS` on the merged SHA. Atlas `7d1bd05b-91bf-4a4a-831f-16b87695d61c` is `SUCCESS` on the exact generated release; installed skill `2.2.1` and MCP asset hashes match source. All three health/readiness probes pass, the gateway runs, and live JSON-RPC proves `27` ordered unique tools with the original `22` unchanged and no forbidden tool.
- Fresh deployed-code verification reads all `178` original notes across `154` contacts correctly and reproduces the exact pre-release source/note hash with no database writes. Authenticated production browser checks confirm contact #149 / note #107 now displays its real text in both recovered and internal Notes, contact #67 shows its true Updated-note title and long body, and contact #89 preserves its title-only content without a false missing-body notice. No migration, archive reapply, or business send occurred.
...
- The exact pinned template `7224d7c1a4dcffe9304f49bc843f55716f5561b4` and Hermes `77a1650c78a4cb1813d8a81fa1da40a15b6a3ec5` gate exposed and fixed a real runtime allowlist mismatch for the two newly advertised tools. The final exact suite passes `299` tests plus `3` subtests, and live-probe code proves `27` ordered unique tools, the original `22` unchanged, required request UUID schemas, and no forbidden approval/send capability.
- Frontend verification passes all `687` component tests, TypeScript, scoped ESLint, the `37`-route production build, `45/45` release browser gates, and `24/24` repeated Contacts accessibility runs. The full four-project browser inventory passes `80`, skips `14`, and retains exactly three unrelated Tasks-page accessibility failures that were reproduced unchanged on clean `origin/main`: six 16px task checkboxes and two assertions over the same 4.49:1 pager-label contrast. Those baseline failures are not called green and are outside this release.
- Focused Ruff, critical whole-tree Ruff, formatting, compileall, YAML syntax, migration guards, diff whitespace, provider-boundary, and credential/PII review pass. Production card sending remains fail-closed even when provider variables are present; the test fake is injection-only. No provider call, external card send, production deployment, migration, or archive reconciliation occurred in this development gate.
...
- Verification passed `46` focused backend timeline tests, `153` focused contact/UI tests, targeted desktop/mobile/axe browser regressions, all `7` Contacts visual checks, TypeScript, scoped ESLint, a clean `37`-route Next.js production build, and diff checks. Darwin and pinned-container captures were pixel-identical; GitHub Ubuntu exposed only 13-15 rasterization pixels, so the final cross-runner budget is capped at 20 pixels per full-page image without relaxing the per-pixel threshold. No archive reconciliation, production deployment, migration, or external action occurred in this task.
...
- The test-first execution plan is recorded at `docs/superpowers/plans/2026-09-04-sydney-command-card-repair.md`, with ten bounded tasks covering runtime, dedupe/acknowledgement, celebrations, card persistence/API/UI, contact repair, verification, and guarded rollout. Planning made no runtime or production change.
...
- The original implementation merged through PR `#29`: reviewed head `5fd07b89d855cf08fb5d472f6810cd752b7eb9f9`, exact main merge `551823e431aca9a4096c53f13d9861c8331af853`, and Atlas deployment `d34f1581-e546-4979-bb78-13ba4ebc0c7e` reached `SUCCESS`. Public health passed, live JSON-RPC returned the exact ordered `25` unique tools with no forbidden tools, and the image and installed managed skill both hashed to `7a8711fab72c32792f0140d2e8c63a243d0d95f67ad2f49519c5ecd4af5b321b`.
- The guarded production selector passed a content-free dry-run and enqueued exactly once. Run `9e2f123a-23db-4c48-9a87-b757e11c5e4a` completed the model turn with one final assistant event and invoked only `command_contact_audience_preview` and `command_contacts_search`; both read-only Command calls reached `succeeded`, every associated spool row drained, and no Gmail, Workspace, calendar, CRM, or Command mutation occurred. Telegram delivery then failed locally before any Bot API request because Hermes attempted to convert the synthetic durable recovery ID into a numeric reply target. The restart watcher correctly preserved the uncertainty as `blocked_side_effect`; Brandon did not receive the review packet.
- PR `#30` separately hardened spool-drain liveness at merge `d0f25096035f459c462efb7ca3b42c1deab8d34e`. Atlas deployment `0d0c8a16-eb74-4b8b-9dc7-824b4114e982` reached `SUCCESS`, public health and the exact live `25`-tool contract passed, and the deployed Sydney provider hash was `9f1e1c1b1f17f7a907786ade18ae8135cdfacabe1030199634352d2f705630f4`. Its `250 passed, 5 skipped, 3 subtests` gate remains valid hardening, but the briefly pending tool-result row drained normally and was not the production delivery root cause.
- The permanent reply-target fix shipped through PR `#31`: reviewed head `1ff4a0e4f59fc281972d3ef9ee37d29061f6d45a`, exact main merge `2ab4096b9a95e3077af9fea2adeb791729afb91c`, and `254 passed, 1 skipped, 3 subtests` against pinned Hermes `77a1650c78a4cb1813d8a81fa1da40a15b6a3ec5`. Atlas deployment `c76a78ba-36d5-4d67-8f6b-efc46a49cfd5` reached `SUCCESS` on image `sha256:cea1eae42da5f98effbb8bdb8e67a17e76c3c68ca5927e5d650e9e0a90ca4949`; backend `8d041172-57b7-4165-a0c1-454ba1469a0b` and worker `6f09282e-834e-407b-896d-0e768628268b` also reached `SUCCESS` on the exact merge. These IDs identify the application-code rollout; a later evidence-only merge may trigger byte-equivalent backend/worker redeploys. The release overlay was byte-stable at relative-tree SHA-256 `49062e4fe09eb7bc9e5ba469c6d46b6ecd44745df4990a750589d656e76842b2`; live installer, provider, runtime, and managed-skill hashes are respectively `2c9b6186da334805ea9c254607d51d3977119923fbf4d69dbfde07ef52cec1ba`, `9f1e1c1b1f17f7a907786ade18ae8135cdfacabe1030199634352d2f705630f4`, `82e36c14aaa9687f1821c75ad4d40407e2ca8a9f891ca61d27baf3bf0ef39592`, and `7a8711fab72c32792f0140d2e8c63a243d0d95f67ad2f49519c5ecd4af5b321b`. Live JSON-RPC still returns exactly `25` ordered unique tools, preserves the original `22`, requires UUID `gmail_send.request_id`, and exposes no forbidden write tool.
- The already-generated review packet was delivered through the independently reviewed, at-most-once repair bytes at SHA-256 `48d8e158559f4a7a9298f82f73638dcfb94dca03dd43e68bd6ef701c26fcb7ba`. Its final content-free preflight proved the exact preserved response hash `77d7f6ed9cd77688fc1615b14ba8e890282c7e614514db4624a97127512c05b8`, two acknowledged successful Command reads, a fully drained outbox, the authoritative pre-network conversion failure, no prior provider attempt, and no mutation. The one authorized Telegram POST returned an exact HTTP `200` receipt; the durable repair record is `confirmed` with provider attempt count `1`, a positive provider message ID represented only by SHA-256 `05ada863a4cf9660fd8c68e2295f1d35b2264815f5b605003d6625bd9e0492cf`, and the uncertainty marker cleared atomically. An immediate identical `send` invocation returned `already_confirmed=true`, `replay_protected=true`, and `provider_attempted=false`. No model rerun, re-enqueue, run reopen, or response rewrite occurred; the repair output and receipt record added no raw prompt, response, chat, platform-message, token, or credential value.
- Fresh Command verification replayed only the stored audience-preview arguments through the read-only backend route. The current result exactly matches the stored result at `362` contacts, checksum `299a069fad83fed13e7ae3259867ac6256366208be33f848b5fb7461da7c46db`, and reference `6c935c78-e842-542d-b335-2a6dea81324e`; the stored audience/search result hashes are `d3100ee774c6cbc60d14a2087ed60217f6bf818ed4741b38f710e7d92dbe7958` and `8aa339f52180f55e6669fccbb1eb817fd8a819a8d9e9e0e3d4b52868600d2fc6`. The recovered run still has exactly those two successful read-tool lifecycles and zero `contacts_search`, Drive/admin-UI, Gmail, Docs/Sheets, Calendar, CRM, or Command mutation tool calls.
- Backend health, worker health/readiness, and Atlas gateway health pass. Durable context has zero checkpoint lag, `14/14` reconciled sessions, and all four feature flags enabled. Its status remains truthfully `degraded` because the delivery-evidence contract intentionally preserves this exact run as the sole `blocked_side_effect`; a direct content-free database check proves projection state `healthy`, zero projection failures/error category, and zero recent actionable terminal failures. The run state and final assistant event remain unchanged, the spool has zero pending rows, and the receipt record—not a destructive state rewrite—is the delivery authority. Sydney Command recovery is complete.
...
- The final application-code rollout deployments reached `SUCCESS`: backend `4a761b35-04b6-4ab2-8774-7ef20c5f072f`, worker `0b3bf3b5-ca82-4d26-b585-0502965fd7df`, and Atlas `34932fbf-e916-4032-97fa-50b9d477a815`. Later evidence-only commits may trigger byte-equivalent backend/worker redeploys, so these IDs identify the code rollout rather than a forever-current deployment. Backend and worker health, worker readiness, Atlas health, and the single gateway process pass. Live JSON-RPC `tools/list` returns exactly 25 ordered unique tools, preserves the original 22, requires UUID `gmail_send.request_id`, and exposes no forbidden write tool.
...
- A final post-documentation deployment gate caught a separate projection issue before closure: a 100-event prompt reached `18911` input tokens, exhausted the strict `4096`-token response ceiling at `4081` output tokens, and returned no parsed object. The identical server-owned range bounded to 50 events stopped normally at `3205` output tokens, echoed every source ID, and passed strict validation. PR `#26` therefore changes only the default per-cycle range `100 -> 50`; it does not relax schema, fact citations, claims, the output ceiling, or raw fallback. Local verification passed `164` context/runtime tests with `52` expected environment-dependent skips. Two consecutive deployed cycles advanced checkpoints/facts `41/35 -> 43/36`, reduced lag `155 -> 55`, reset failures `7 -> 0`, released every claim, and kept protected status `ready`.
...
- Historical Task 8 authentication note: the then-saved Railway token was unauthorized, so that rollout used the directly authenticated Member CLI with both token variables unset. Task 14 used a separately supplied account/workspace token verified with Member SSH/deployment access on 2026-08-26; it was not persisted over the older ignored `.env.railway-sweeney.local` value. The wrapper or direct CLI is valid only when that same path can list and access `enchanting-perception`. The backend service ID is `37afead7-c55b-4ef2-8071-ca4da1c88992`.
