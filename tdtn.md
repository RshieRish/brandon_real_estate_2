...
### 2026-08-27 - Sydney Command Recovery Rollout Through Reply-Anchor Fix Development Gate
...
- The actual reply-target defect is fixed test-first on `codex/sydney-recovery-reply-anchor`: Telegram reply anchors now accept only positive ASCII decimal message IDs, discard synthetic durable IDs, and leave non-Telegram reply behavior unchanged. The exact changed-area matrix passes `254` with `1` expected exact-checkout skip and `3` subtests; focused Ruff, compileall, and diff checks pass. No packet has been resent. Final recovery remains incomplete until this fix is reviewed, merged, deployed, and the already-generated response is delivered exactly once under a durable receipt guard without enqueuing another model run.
