...
function parseRequest(
  ...
  const pageSize = integer(params.get('page_size'), 1, 100) ?? DEFAULT_REQUEST.page_size;
...
function mergeRequest(
  ...
  return {
    ...
    page_size: bounded(next.page_size, 1, 100) ?? DEFAULT_REQUEST.page_size,
...
export function useContactDirectoryQuery(
  ...
  const parsedRequest = useMemo(
    () => parseRequest(new URLSearchParams(searchString), initialView),
