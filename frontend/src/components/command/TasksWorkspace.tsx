...
export function TasksWorkspace() {
  ...
  async function add() {
    ...
    try {
      const task = await commandApi.createTask({ title: title.trim(), description: '', priority, contact_id: null, due_at: dueAt ? new Date(dueAt).toISOString() : null });
      setTasks((all) => [...all, task]); setTitle(''); setDueAt('');
