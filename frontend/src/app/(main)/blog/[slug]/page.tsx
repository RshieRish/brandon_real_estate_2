...
// Custom markdown components styled for Brandon's brand
  ...
  useEffect(() => {
    ...
    const fetchBlog = async () => {
      try {
        ...
        if (!res.ok) {
          if (res.status === 404) { router.replace('/blog'); return; }
    ...
    return (
      <div className="min-h-[100dvh] bg-dark-surface flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 text-center px-6">
          <WarningCircle weight="fill" className="w-10 h-10 text-red-400" />
          <p className="text-white/60 text-lg">{error ?? 'Article not found'}</p>
          <Link href="/blog" className="text-gold text-sm hover:underline flex items-center gap-1">
