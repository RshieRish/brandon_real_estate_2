...
import { commandApi, type ContactImportRow } from '@/lib/command/api';
...
export default function ImportContactsPage() {
  ...
  async function loadFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const parsed = file.name.toLowerCase().endsWith('.json') ? validateRows(JSON.parse(source)) : parseCsv(source);
      if (parsed.length > 10000) throw new Error('Imports are limited to 10,000 contacts per file.');
      setRows(parsed); setError(''); setResult('');
    } catch (err) { setRows([]); setError(err instanceof Error ? err.message : 'Unable to read that file.'); }
    event.target.value = '';
  }
  ...
  return <div className="min-h-[100dvh] bg-[#080807] p-6 text-white"><main className="mx-auto max-w-4xl"><p className="text-xs uppercase tracking-[.25em] text-[#eac469]">Permitted internal intake</p><h1 className="mt-1 text-3xl font-black">Import contacts</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-white/55">Bring a permitted contact archive into the canonical CRM. CSV or JSON only. Required: <code>first_name</code>. Optional: <code>last_name, email, phone, stage, birthday, anniversary</code>. Files stay in this admin workflow and are sent only to the internal CRM API.</p><label className="mt-8 flex min-h-44 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[#eac469]/50 bg-[#eac469]/[.04] p-8 text-center transition hover:bg-[#eac469]/[.08]"><input className="sr-only" type="file" accept=".csv,application/json,.json,text/csv" onChange={loadFile}/><FileArrowUp size={32} className="text-[#eac469]"/><b className="mt-4">Choose a CSV or JSON contact file</b><span className="mt-2 text-sm text-white/45">Up to 10,000 contacts; the importer sends secure batches of 1,000.</span></label>{error && <p role="alert" className="mt-5 flex gap-2 rounded-xl border border-red-300/20 bg-red-300/10 p-4 text-sm text-red-200"><WarningCircle size={18}/>{error}</p>}{rows.length > 0 && <section className="mt-6 rounded-2xl border border-white/10 bg-white/[.035] p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="font-bold">{rows.length.toLocaleString()} contacts ready</h2><p className="mt-1 text-sm text-white/45">Previewing the first {preview.length} rows. Existing emails are skipped case-insensitively.</p></div><button disabled={running} onClick={importRows} className="inline-flex items-center gap-2 rounded-xl bg-[#eac469] px-4 py-3 text-sm font-bold text-black disabled:opacity-50"><UploadSimple size={18}/>{running ? 'Importing…' : 'Import contacts'}</button></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm"><thead className="text-xs uppercase tracking-widest text-white/40"><tr><th className="pb-3">Name</th><th className="pb-3">Email</th><th className="pb-3">Phone</th><th className="pb-3">Stage</th></tr></thead><tbody>{preview.map((row, index) => <tr key={`${row.email}-${index}`} className="border-t border-white/10"><td className="py-3">{row.first_name} {row.last_name}</td><td className="py-3 text-white/60">{row.email ?? '—'}</td><td className="py-3 text-white/60">{row.phone ?? '—'}</td><td className="py-3 text-white/60">{row.stage}</td></tr>)}</tbody></table></div></section>}{result && <p className="mt-5 rounded-xl border border-[#eac469]/25 bg-[#eac469]/10 p-4 text-sm text-[#f5d98f]">{result}</p>}</main></div>;
}
