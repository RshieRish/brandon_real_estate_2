...
test('global search remains accessible while modal focus is contained', async ({ commandPage }) => {
  ...
  await expect(commandPage.getByRole('combobox', { name: 'Search Command' })).toBeFocused();
...
test('keyboard semantics expose skip, navigation, task tabs, and record rows', async ({ commandPage }) => {
  await commandPage.goto('/admin/command');
  await expect(commandPage.getByRole('heading', { name: 'Follow-Up Readiness' })).toBeVisible();
  ...
  const allTasks = commandPage.getByRole('tab', { name: 'All' });
  await expect(allTasks).toHaveAttribute('aria-selected', 'true');
  await allTasks.focus();
  await commandPage.keyboard.press('ArrowLeft');
  await commandPage.keyboard.press('Enter');
  await expect(commandPage.getByRole('tab', { name: 'Team' })).toHaveAttribute('aria-selected', 'true');

  await commandPage.goto('/admin/command/contacts');
  const table = commandPage.getByRole('table');
  await expect(table).toBeVisible();
  const firstRecord = table.getByRole('link', { name: 'Avery Lake' });
