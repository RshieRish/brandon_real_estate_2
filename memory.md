...
- 2026-06-02: Local Atlas/Hermes backend now includes the deeper Workspace action-tool slice pending commit/deploy: `workspace.gmail.search`, `workspace.gmail.thread.read`, `workspace.drive.file.read`, `workspace.calendar.events.read`, `workspace.calendar.event.create`, and `workspace.contacts.search`. Calendar event creation requires `confirmed_by_brandon=true`; audit metadata excludes Gmail bodies, Drive file contents, contact addresses/phones, calendar descriptions, and attendee addresses. Focused backend tests and route smoke passed locally; still need push, Railway success, live action-catalog smoke, Telegram channel/allowlist, and Hermes-side tool invocation wiring.
