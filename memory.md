...
- 2026-06-02: Brandon approved broad Workspace access for Hermes/Atlas. Implemented a full-access Workspace OAuth connector with routes under `/api/v1/workspace/*`, durable token key `google_workspace_refresh_token`, and Settings UI connect/reconnect controls. The connector requests Gmail full mail access, Calendar, Drive, Docs, Sheets, Slides, Forms, Contacts/Directory, Tasks, Chat, Meet, and Admin SDK scopes. This grants capability only after Brandon consents; autonomous send/delete/edit tools still need explicit backend agent-tool implementation and confirmation policy.
