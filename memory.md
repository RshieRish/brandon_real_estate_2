...
- 2026-06-01: `atlas-agent` is not created yet. `scripts/railway-sweeney add --service atlas-agent --repo praveen-ks-2001/hermes-agent-template` was rejected as unauthorized with the current project-scoped token. Use the Railway dashboard or a dedicated account-wide `RAILWAY_API_TOKEN` to create the new service, then mount `/data` and set admin credentials.
