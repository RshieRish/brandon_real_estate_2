...
- 2026-06-02: Current verified Hermes/Atlas state is not PRD-complete yet. Backend health is OK, Workspace OAuth is connected, Hermes health reports gateway running, and `/api/v1/agent-control/status` is production OK, but agent-control capabilities are still read-only (`status.read`, `leads.recent.read`, `bookings.recent.read`). Remaining work beyond Telegram token/user ID: configure the Hermes channel/allowlist and implement explicit Workspace action tools for Gmail, Drive, Docs, Sheets, Calendar, Contacts, etc. before claiming full executive-assistant automation.
