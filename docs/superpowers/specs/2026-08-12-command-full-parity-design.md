...
- **Contacts:** 317 captured contact positions with 317 unique upstream provider IDs and the required eight-view matrix: Timeline, Opportunities, SmartPlans, Notes, Saved Searches, and Tasks in To Do/Completed/Archived states. The immutable identity audit resolves 317 recovered identities and coalesces zero aliases. The existing 313 source-normalized/311 leadless database rows are stale legacy-import audit history that must be repaired in place; they are not the recovered source target and must not be deleted blindly. The database also contains 51 lead-backed contacts, including two strong verified overlaps with recovered identities, leaving 49 legacy-only contacts and an expected combined directory total of 366. The planned Task 4 flow supplies those two overlaps through a private, fingerprint- and parser-bound manifest with hashed source/evidence identities, positive existing `CRMContact.id` targets, and non-PII target-row fingerprints. V1 has no alternate target key. Once implemented, apply persists the exact source records, validates strong evidence and unchanged target fingerprints against the lead-backed targets, then creates two audited reviewed links in the same Contacts transaction before materialization. The materializer adopts those links and creates the remaining 315 recovered mappings. The import never auto-merges an unreviewed lead-backed contact and never changes a lead-backed contact field. This manifest/materializer flow is not available in the current CLI, so Contacts apply remains blocked.
...
The current importer supports `--dry-run`, `--apply`, `--resume`, `--module`, and `--verify-only`, and its default registry contains `archive_integrity` plus `contacts`; verify-only selects only archive integrity, while unbounded dry-run selects both. The current CLI does **not** support `--contact-overlap-manifest` and has no Contacts materializer, so Contacts apply and unbounded apply are blocked. Task 4 will add the private flag/loader, exactly two hashed reviewed mappings bound to the bundle fingerprint/parser version, positive existing `CRMContact.id` targets with non-PII row fingerprints, and redacted logging. Only after that work is deployed may manifest-aware dry-run validate without semantic writes and apply persist source records, revalidate/stage two audited source/entity links, then materialize the remaining 315 mappings inside one transaction. Any failure must roll back all module writes, and resume must repeat exact source/target/evidence validation against the run fingerprint/parser version. These Task 4 commands are not available today.
