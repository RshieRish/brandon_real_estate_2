...
- [x] **Step 1: Write deterministic end-to-end tests** covering three independent workflows:
...
- [x] **Step 2: Wire job schedules** for history polling, receipt processing, Sydney questions, and integration alerts. Use deterministic job keys and leased claims.
- [x] **Step 3: Expand the dedicated workflow rather than replacing earlier jobs.** Pin its final PostgreSQL list to every backend persistence/concurrency/E2E test in Tasks 1–6 and 9; retain the Task 7 exact Vitest/typecheck/scoped-ESLint job and the Task 8 exact MCP/overlay/22-tool job. Keep `hermes/**`, both frontend package files, and every exact Task 7 source/test path in triggers. Run the focused backend/frontend suites and record the known-red full frontend lint baseline separately without treating it as a green gate.
- [x] **Step 4: Run the deployment preflight:** prove the CRM task-foundation Tasks 7/8 evidence is green; capture intended backend/worker/frontend/Hermes commits and deployment IDs; check backend `/health`; check the worker's exact `/health` and `/ready` JSON with `backend/scripts/check_integration_worker.py` (not curl); verify sole Alembic head `84d7a5f9b2c3`; OAuth profile/current History access; Telegram `getMe`; configured outbound chat allowlist; protected Gmail status; deployed authenticated Task 7 handoff exchange/URL removal; and the live deployed `tools/list` gate from Task 8. A configured Telegram user ID is recorded only as future-gate configuration, not current trusted inbound proof.

  **Completion evidence (2026-08-24):** PR #7 merged as `bb66b6290e882c8da90555cd3a011548e965c351`; the exact main-branch PostgreSQL, Task 7, and Task 8 checks passed. Railway backend deployment `a44293d7-7bfd-48a9-a378-889e0ec19806` and dedicated worker deployment `e6fccf87-c826-4b89-a5f6-29f3db8ddfd5` reached `SUCCESS` on that merge. The worker used root `/backend`, config `/backend/railway.integration-worker.json`, `Dockerfile.worker`, `python -m workers.integration_worker`, `/health`, a 30-second health timeout, and three on-failure retries. Vercel deployment `HgQL9QEMhT7QBoYp2r1aj17yZcSC` reported success for the same merge. Atlas deployment `51f50a75-32ed-4ed7-bd88-5dc5bfde0988` remained `SUCCESS` on Task 8 source `a53ff04` with pinned Hermes `7224d7c`.

  Backend `/health` returned the exact expected service/status object. The repository-owned worker probe passed `/ready`, and `/health` returned the exact expected worker object. Live Alembic `current` and `heads` both returned sole head `84d7a5f9b2c3`. The bound Workspace credential passed Gmail profile identity and current History access without exposing the mailbox or token. Telegram `getMe` passed; exactly one outbound chat and one future-only user entry were configured, with no all-users setting. Unauthenticated Gmail status returned `401`; authenticated status returned `200`, one bound account, zero active runs and pending receipts, and a current worker heartbeat while Gmail/question delivery remained disabled. The previously completed authenticated Task 7 handoff exchange/separate-approval proof was carried forward; the exact Task 9 Vercel deployment loaded the authenticated review workspace and synchronously removed a fresh fragment marker from the URL, with the CRM task count unchanged at `453`. Fresh deployed JSON-RPC `tools/list` returned exactly 22 ordered unique tools, preserved the original 16, required UUID `gmail_send.request_id`, and exposed none of the five forbidden trusted-write tools. `GMAIL_TASK_INTAKE_ENABLED=false`, `SYDNEY_TASK_QUESTIONS_ENABLED=false`, and `INSTAGRAM_INTEGRATION_ENABLED=false` were confirmed in both backend and worker runtimes. No Gmail or Telegram message was sent and no task was created during preflight.
- [x] **Step 5: Run controlled production E2Es with unique UUID fixture IDs and Brandon's explicit approval.** Send one controlled received email into the mailbox, one controlled Agent Control sent commitment, and one direct Sydney draft. For each, record the redacted send-intent state transition, Gmail message/thread IDs, receipt/suggestion IDs, source counts, clarification/outbox/Telegram message IDs where applicable, handoff and approval nonce consumption timestamps (never nonce values), final preview hash, task ID, and audit ID. Query counts before/after and replay each input to prove exactly one task. Do not use a client conversation or enable autonomous creation.

  **Completion evidence (2026-08-24):** Brandon explicitly authorized and approved all three controlled production fixtures. Received fixture `982c69dd-b76b-4545-b0b8-1f30b255401c` produced Gmail message/thread `1a034d7ddfca2791`, receipt `eb82b845-c616-421f-b5d1-dba955e99b4f`, suggestion `f9a333f6-0b3e-4e05-bb96-f8494adfd282`, one Gmail source, final hash `80309124002cb9752b75845b82b73c246489250da914a02a5289caba2ea207cc`, edit/approve audits `215`/`216`, approval issuance/consumption at `18:05:00.294170Z`/`18:06:18.050819Z`, and exactly Task `454` with one creation request and one task source. Agent Control sent request `1100422e-4074-42c0-b7a3-08eb6268e0dd` replayed to message/thread `1a034d7e937e7ac2`; origin `42573720-3b0c-4035-87e3-6e39eb25318d` succeeded under audit `187`; receipt `4db4edae-e040-4d6b-ab25-4611428948f3`, suggestion `f592e23a-4692-4c24-b9fc-ed2c24a0fea6`, one Gmail source, final hash `fea579ba02be818178b2d6d7fbf12e95d2124a893bd1e419580091dae643736f`, edit/approve audits `217`/`218`, approval issuance/consumption at `18:08:13.903982Z`/`18:08:23.987464Z`, and exactly Task `455` each remained singular. Direct Sydney request `30277959-206a-4929-b1d7-676820d323aa` replayed to suggestion `7e10f85b-fe97-416d-ac4b-d1386a2c38a9`; clarification `420aaf01-6663-4439-8aac-404f8166f799` and outbox `36ad52c7-9346-4b54-90b1-18673ea3566d` sent Telegram message `88` once; draft/answer/link/approve audits were `219`/`220`/`235`/`236`; the successful final handoff was issued/consumed at `19:28:55.997543Z`/`19:29:28.068269Z` and its separate approval was issued/consumed at `19:29:28.068269Z`/`19:29:35.457930Z`; final hash `712501a991739de609fd651243950344b2c64ec9eb2754234a9a931941dcf3b7` applied exactly Task `456` with one creation request and task source. Replays of every controlled input produced no duplicate. The absolute-link defect discovered during the handoff was fixed in `68d4db2`, merged by PR #11 as exact main `ca544e6dc9e99c08b3cd9b2c498eefd0774d1cf5`, and deployed to backend and worker. Final totals were 456 tasks, 16 suggestions, three creation requests, three task sources, six origins, 34 receipts, 47 attempts, 13 obligations, one clarification, and one outbox row. All rollout flags were then explicitly reset to false on both services; backend deployment `c46df9ff-d4c1-48a5-8b3b-adb29e3b43af` and worker deployment `e4efa174-0d7b-4623-8f05-afbb8246d980` reached `SUCCESS` on `ca544e6`, both health gates passed, and the worker heartbeat had no current job. A later unrelated provider-deleted Gmail message remains a pending, alerted `message_not_found` incident; it was not acknowledged without exact provider-404 verification and Gmail intake remains disabled.
- [x] **Step 6: Update deployment docs, `tdtn.md`, and `memory.md`** with exact environment variables, Railway worker command and both probes, enable/disable switches, final-page cursor/reseed/backfill runbook, Telegram uncertain-delivery reconciliation, live `tools/list` evidence, rollback, and remaining blockers.
- [x] **Step 7: Commit:** `docs: document Gmail Sydney task intake`.
