...
- [x] **Step 2: Push `codex/sydney-durable-context`, open a PR with design/plan/testing/rollout details, wait for required checks, address review test-first, and merge only with the reviewed head SHA.** Feature PR `#14` and test-first rollout corrections through PR `#26` passed their required checks and used SHA-pinned merges; the final application-code merge is `de986f5e9c3808fd1253c2532670bfd9f7ce9e65`, followed only by evidence documentation.
- [x] **Step 3: Create a mode-0600 PostgreSQL custom-format backup outside the repository, validate its catalog, record only its path/size/hash, and verify production `alembic current` is `84d7a5f9b2c3` before applying sole head `85e8b7c9d4f1`.** The preserved backup is mode `0600`, `516102115` bytes, SHA-256 `8a200bba3ca51af9eba43e028a838b90eaaf4ef51553a33e77c8c7b7a2252847`, with a valid `732`-entry PostgreSQL 17 catalog; production now reports sole head `85e8b7c9d4f1`.
- [x] **Step 4: Deploy backend and worker with all four new flags false.** Staged deployments passed before promotion. Application-code rollout backend `4a761b35-04b6-4ab2-8774-7ef20c5f072f` and worker `0b3bf3b5-ca82-4d26-b585-0502965fd7df` reached `SUCCESS`; later evidence-only merges may create byte-equivalent deployment IDs. Public health, worker `/ready`, database, migration, heartbeat, and job-registry checks pass.
- [x] **Step 5: Enable master write-only shadow mode for Brandon, deploy the exact-pinned Atlas image, and inspect the live runtime.** Final Atlas `34932fbf-e916-4032-97fa-50b9d477a815` is `SUCCESS`; one gateway runs, the private spool exists, and live JSON-RPC returns exactly 25 ordered unique tools with the original 22 unchanged and no forbidden write tool.
- [x] **Step 6: Run backfill inside `atlas-agent` and repeat until reconciliation reports no unacknowledged rows.** The compacted proof matches `8` sessions, `918` messages, `947` events, role counts, `455/455` tool calls/results, every session proof, and global hash `9c7c960a59b959e13faa317fda987193d60b8416367212da249c4281a73be689`, with zero unacknowledged rows.
- [x] **Step 7: Enable retrieval canary and run benign production checks:** automatic recall remained source-linked under `16000` tokens across continuation/restart; bounded history search returned event-ID-linked excerpts; Command-only search/audience preview returned an exact checksummed audience with masked samples and only Command sources. No Google Contacts substitution, UI scraping, email, calendar, CRM mutation, or write tool occurred.
- [x] **Step 8: Enable projection, wait for a valid source-linked checkpoint, and verify retrieval remains correct if projection is then temporarily disabled.** Raw fallback remained source-linked. PR `#23` safely bound the production model's one-interior-ID echo omission. A later 100-event range then exhausted the strict output ceiling; PR `#26` reduced only the default immutable range to 50 events. Two consecutive deployed cycles advanced checkpoints/facts `41/35 -> 43/36`, reduced lag `155 -> 55`, reset failures `7 -> 0`, released every claim, and kept context health `ready`.
- [x] **Step 9: Enable retry canary and inject a safe synthetic short `429`.** One saved `45s` wait and acknowledgement survived an Atlas restart; attempt `2` produced one exact final assistant event and reference, zero tool invocations, no duplicate side effect, and no unresolved state. The spool and synthetic marker were cleaned after proof.
- [x] **Step 10: Enable full durable context for Brandon, monitor content-free health, add exact deployment/reconciliation/acceptance evidence to `tdtn.md`, `memory.md`, and the runbook, commit/push the documentation evidence if needed, and leave rollback switches documented.** Owning-service flags are fully promoted, all `14` live sessions are reconciled, protected status is `ready`, rollback remains evidence-preserving, and Task 14 production evidence is recorded without private IDs or transcript text.
