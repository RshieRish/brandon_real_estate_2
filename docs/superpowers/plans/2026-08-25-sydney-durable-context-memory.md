...
- [x] **Step 4: Deploy backend and worker with all four new flags false.** Staged deployments passed before promotion. Final exact-main backend `aa4a1ae3-36a5-4524-9e96-5d0d91223836` and worker `957d12c3-dfbe-4470-877d-d2e26e5b929c` are `SUCCESS`; public health, worker `/ready`, database, migration, heartbeat, and job-registry checks pass.
...
- [x] **Step 8: Enable projection, wait for a valid source-linked checkpoint, and verify retrieval remains correct if projection is then temporarily disabled.** Raw fallback remained source-linked. After PR `#23` fixed the production model's one-interior-ID echo omission, checkpoints advanced `16 -> 17`, facts `14 -> 15`, failures reset to zero, the claim released, and context health returned `ready`.
